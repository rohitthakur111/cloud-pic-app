  ...
  return (
    <div className="flex justify-center mt-6">
      <div className="flex items-center space-x-2 bg-white shadow-lg rounded-lg p-3">
        ...
        {/* Page Numbers */}
        {Array.from(Array(totalPages ?? 1).keys())?.filter((_,i)=>( i+1<4 || i+3 >= totalPages || i+1 === currentPage-1 || i+1 === currentPage || i+1 === currentPage+1)).map((val,i,arr)=>(
          <div key={i} className='flex gap-2'>
          {((currentPage > 5 && i === 3) || (currentPage < totalPages-4 && val === totalPages-3)) && <span className='flex items-end' ><BiDotsHorizontalRounded /></span>}
          <button 
              key={i} 
